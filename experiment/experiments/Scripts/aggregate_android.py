...
def aggregate_android_final(logs_dir):
    ...
    # FIXME I just added the 'android_' prefix because of a KeyError... this is just a bandage solution (Ruben)
    runs_total = reduce(lambda x, y: {k: v + y['android_' + k] for k, v in list(x.items())}, runs)
    return OrderedDict(
